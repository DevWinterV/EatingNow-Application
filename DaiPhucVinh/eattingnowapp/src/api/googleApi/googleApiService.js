...
const searchAddressGGAPI = async (searchQuery) => {
  const encodedSearchQuery = encodeURIComponent(searchQuery);
  const apiUrl = `http://localhost:3002/search-address?searchAddress=${encodedSearchQuery}&region=vn&key=${apiKey}`;
  try {
    const response = await axios.get(apiUrl);
...
export {
    handleSearchAddress,searchAddressGGAPI
}