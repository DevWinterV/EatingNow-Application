...
import { TakeStoreByCuisineUserLogin, TakeStoreByUserLogin } from "../../api/store/storeService";
import { Linking } from "react-native";
import { TouchableOpacity } from "react-native";
// import * as Linking from "expo-linking";

export default function StoreByCuisine({ navigation, route }) {
  const { dataCuisine } = route.params;
  ...
  async function getLocation() {
    ...
    let response = await TakeStoreByCuisineUserLogin({
      latitude: currentLocation?.coords?.latitude,
      longitude: currentLocation?.coords?.longitude,
      Count: 20,
      CuisineId: dataCuisine.CuisineId,
    });
    if (response.success) {
  ...
  return (
    <Div bg={"#F2F4F5"} flex={1}>
      <Div p={24} bg="body">
        <ClearNavigation title={`Cửa hàng ${dataCuisine.Name} gần đây`} hasBack={false} />
      </Div>
