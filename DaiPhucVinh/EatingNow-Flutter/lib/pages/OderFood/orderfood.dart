...
class _OrderPage extends State<OrderPage> {
  ...
  final _streamCartlist = StreamController<List<CartItem>>.broadcast();
  final _streamCallBack = StreamController<bool>.broadcast();
  ...
  void _loadCartItems() async {
    ...
    _streamCartlist.sink.add(cartItem);
  }
  ...
  initState() {
    ...
    _streamCallBack.sink.add(false);
    _loadCartItems();
    _loadUserData();
    _loadPaymentMethod();
  }
...
// Method to reload content
  ...
  Widget build(BuildContext context) {
    // Đăng ký lắng nghe sự kiện của stream và gọi reloadContent() khi result là true
    _streamCallBack.stream.listen((result) {
      if (result) {
        print("Lăng nghe thay đổi");
        reloadContent();
      }
    });
    return
      StreamBuilder<List<CartItem>>(
        stream: _streamCartlist.stream,
        builder: (builder, snapshot){
          if(snapshot != null && snapshot.data!.length > 0){
            final cartItem = snapshot.data ?? [];
            double totalAmount =  cartItem.fold( 0, (total, item) => total! + (item.price * item.qty)) ?? 0;
            return Scaffold(
                appBar: AppBar(
                  title: Column(
                    children: [
              ...
              child: Center(
                child: 
                    Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        BigText(text: "Bạn đã xóa các sản phẩm trong đơn đặt hàng", size: Dimensions.font16, color: AppColors.mainColor,),
                        SmallText(text: "Tiếp tục mua sắm nào", size: Dimensions.font13, color: AppColors.mainColor,),
                        ElevatedButton(
                          style: ElevatedButton.styleFrom(
                            // Customize the background color
                            foregroundColor: Colors.white,
                            ...
                            minimumSize: Size(40, 40), // Đặt kích thước tối thiểu cho nút
                          ),
                          onPressed: () {
                            Navigator.of(context).pop();
                          },
